...
Class Check extends Connection
{
	...
	{	
		if($this->connectdb("db_recordings"))
		{
			...
			$result= mssql_query("select * from rec_user where username='$user' and password='$password' and is_logged!='1'")or die("ERROR!");
		  		 ...
		  		 $result= mssql_query("select * from rec_user where username='$user'")or die("ERROR!");
				    ...
				    if ($count==1)
						{	
							...
							$user = $_COOKIE['user'];
						 	$sql="UPDATE rec_user
							SET is_logged='1',act_date='$rdate'
							WHERE username='$user'";
						 	if($result=mssql_query($sql))
						 	{
						 		...
						 		return FALSE;
						 		echo'<div class="counter">'. "activity error!".'</div>';
						 	}
	 ...
	 if($this->connectdb("db_recordings"))
		{
			...
			{	
				...
				$info_search=$search;
				$search_string=$this->escape_string($info_search);
				$info = $search_string."_".$start."_".$end;
				$sql="insert into rec_logs(userId,logDate,logDetail,otherDetail) values('".$id."',GETDATE(),'".$activity."','".$info."')"; 
				  mssql_query($sql);
	 ...
	 { 
		...
		if($search!= NULL)
		{
				if($escape_search!= NULL && $escape_start!= NULL && $escape_end!=NULL)
					{
							//$sorter.= " RecordingDate ";
							$query="calldetail.LocalUserId ='$escape_search' and calldetail.InitiatedDate>='$escape_start' and calldetail.InitiatedDate <='$escape_end'";
							$sql="
										select RemoteNumber as Phone_Number,
										RecordedCallIDKey as CallId,
										RecordingFileName,
										RecordingTitle as CallTitle,
										RecordingFileName,
										RecordingFileSize as File_Size,
										InitiatedDate 
									from calldetail 
										inner join RecordingCall on calldetail.CallId = RecordingCall.RecordedCallIDKey 
										inner join RecordingData on RecordingCall.RECORDINGID = RecordingData.RECORDINGID 
									where $query
									order by RecordingDate,RemoteNumber,RecordingFileSize
								";
				$result=mssql_query($sql);
			 	$count=mssql_num_rows($result);
			 	$counter="<b>Result</b>".":"." ".$count." "."Recordings Found!";
 ...
 function activity_of($user)
	 {
	 	if($this->connectdb("db_recordings"))
		{
			...
			$user = $_COOKIE['user'];
			$sql="UPDATE rec_user
			SET is_logged='0',act_date='$rdate'
			WHERE username='$user' and is_logged='1'";
		 	if($result=mssql_query($sql))
		 	{
		 		return TRUE;
		 	}else{
	 ...
	 function is_logged_checker()
	 {
	 	if($this->connectdb("db_recordings"))
		{
			$user = $_COOKIE['user'];
			$sql="SELECT is_logged FROM rec_user WHERE username='$user'";
			$result=mssql_query($sql);
			while($row=mssql_fetch_array($result))
			{
				if($row['is_logged']==1){
	 ...
	 function check_idle()
	 {
		...
		{
				$sql="UPDATE rec_user
				SET is_logged='0',act_date='$date'
				WHERE datediff(n,act_date,'$date')>='$settime' and is_logged='1' ";
				if($result=mssql_query($sql))
				{
					echo "Login Reset";
					return TRUE;
				}else{
					echo "Idle Update Error!";
				}
		}
