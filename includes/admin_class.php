	...
	function manageuser2()
	 {
		...
		//Now you can use this query to see how many rows you are dealing with
//Edit $result as your query
		$result = mssql_query ("select rec_logs.logId from rec_user INNER JOIN rec_logs ON rec_user.id = rec_logs.userid") or die(mssql_error());
		$rows = mssql_num_rows($result);
...
//the way the table is designed is, "id" is the unique id, and "name" is just a list of names i have in there.
	$result2 = mssql_query("
							select top $page_rows userId,logId,username,logDetail,logDate
							from rec_user INNER JOIN rec_logs ON rec_user.id = rec_logs.userid
							where logid not in 
							(select top $max logid from rec_logs order by logdate asc) 
	...
	function manageuser_search($search,$rad_id,$sdate,$edate)
	 {
		...
		if($search!= NULL)
		{
			if($this->connectdb("db_recordings"))
			 {
				...
				{
					$sorter.= "logDate";	
					$var.= "username ='$search'  AND logDate>='$sdate' AND logDate>='$edate' OR  otherDetail like '%$search%'  ";
				...
				//$result = mssql_query ("select 
										//rec_logs.logId from rec_user 
										//INNER JOIN rec_logs ON rec_user.id = rec_logs.userid
										//where $var 
		    ...
		    // $max = ($pagenum - 1) * $page_rows;
			   ...
			   //This is your query again, just spiced up a bit
				//mssql doesnt have that nice limit ability like mysql... so we use this to make it work...
				//the way the table is designed is, "id" is the unique id, and "name" is just a list of names i have in there.
			$sql="select userId,logId,username,logDetail,logDate
									from rec_user INNER JOIN rec_logs ON rec_user.id = rec_logs.userid
									where $var 
									order by $sorter desc";
									
			$result2 = mssql_query($sql) or die(mssql_error());
